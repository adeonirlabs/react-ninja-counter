...
const button = cva(['flex items-center justify-center rounded-xl text-white transition focus:outline-none'], {
  variants: {
    variant: {
...
export function Button({ variant, icon, children, className, ...props }: ButtonProps) {
  ...
  return (
    <button className={cn(button({ variant, className }))} type="button" {...props}>
      <Icon size={20} />
      {children}
    </button>
  )
